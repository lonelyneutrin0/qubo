     ...
     def anneal(self, Q:ndarray)->solution: 
          ...
          for step in range(self.iterations): 
               ...
               for ml_step in range(self.ml):
                ...
                # Stochastic Parameter Energy Difference
                diff = np.exp(-step*(iter_energy-candidate_energy)*(10**-3)) - np.exp(-step*(temp_energy-candidate_energy)*(10**-3))
                
                # Analyze new states by the Metropolis condition
                if( (diff) < 0 or np.exp(-(diff)/temperatures[step]) > random_numbers[step, ml_step]):
                    self.X = temp_X
                    iter_energy = temp_energy
                    accepts[step] = 1
                else: 
                    rejects[step] = 1
                if temp_energy < iter_energy: 
                    candidate_energy = temp_energy
